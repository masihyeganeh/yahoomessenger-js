...
module.exports.newPacket = function newPacket(serviceID, userStatus) {
  if (typeof userStatus === 'undefined') {
    userStatus = this.sessionData.userStatus;
  }
  return (
    new Packet(serviceID, userStatus, this.sessionData.sessionID)
  );
};
...
module.exports._download = function _download(ip, path, query, callback) {
  ...
  request.on('error', function(e) {
      console.error('Got HTTP error: ' + e.message);
    });
...
module.exports.login = function login(username, password) {
  ...
  var packet = this.newPacket(Service.HELO);
...
module.exports.keepAlive = function keepAlive() {
  var packet = this.newPacket(Service.KEEP_ALIVE);
...
module.exports.ping = function ping() {
  var packet = this.newPacket(Service.PING);
  this.sendPacket(packet);
  return this;
};
...
{
  var packet = this.newPacket(Service.USER_SEND_MESG);
  packet.fields
...
module.exports.chatLogin = function chatLogin() {
  var packet = this.newPacket(Service.CHAT_MSGR_USER_LOGIN);
  packet.fields.add(
...
module.exports.chatJoin = function chatJoin(roomID, roomName, captcha) {
  var packet = this.newPacket(Service.CHAT_ROOM_JOIN)
    .add(Field.CURRENT_ID, this.sessionData.userData.username)
...
module.exports.chatLogout = function chatLogout() {
  var packet = this.newPacket(Service.CHAT_MSGR_USER_LOGOFF);
  packet.fields.add(Field.CURRENT_ID, this.sessionData.userData.username)
...
module.exports.HELO = function HELO() {
  ...
  var username = me.sessionData.userData.username;
  var password = me.sessionData.userData.password;

  var request = 'src=ymsgrb&login=' + encodeURIComponent(username) +
        '&passwd=' + encodeURIComponent(password) +
        '&v=2';
        // +'&chal=' + encodeURIComponent(me.sessionData.userData.challengeStr);
      ...
      if (errorCode === '0') { // Token gotten successfully
        ...
        httpsClient = https.request({
          ...
          res.on('data', function(response) {
            ...
            {
              me.sessionData.userData.crumb = response[0].substr(6);

              me.sessionData.userData.crumbHash = crypto.createHash('md5').
                                      update(
                                        me.sessionData.userData.crumb +
                                        me.sessionData.userData.challengeStr
                                      ).digest('base64') .
                                      replace(/\+/g, '.') .
                                      replace(/\//g, '_') .
                                      replace(/=/g, '-');
              me.sessionData.userData.cookies.Y = response[1].substr(2);
              me.sessionData.userData.cookies.T = response[2].substr(2);

              me.sessionData.userData.cookies.B = response[4].substr(2);
              me.sessionData.userData.cookies.SSL = response[5].substr(4);

              var packet = me.newPacket(Service.USER_LOGIN_2, 1515563605);
              packet.fields.add(Field.CURRENT_ID, username)
                .add(Field.USER_NAME, username)
                .add(
                  Field.LOGIN_Y_COOKIE, me.sessionData.userData.cookies.Y
                )
                .add(
                  Field.LOGIN_T_COOKIE, me.sessionData.userData.cookies.T
                )
                .add(Field.CRUMB_HASH, me.sessionData.userData.crumbHash)
                .add(Field.COOKIE, me.sessionData.userData.cookies.B)
              //.add(Field.COOKIE, 'F')
              ...
              START_OF_RECORD 508
                SYMANTEC_MSGS B8A38602E4AD
                510 0
              END_OF_RECORD 508

              packet.fields.add(508, new PacketParameters([
                Field.SYMANTEC_MSGS, 'B8A38602E4AD', 510, '0'
              ], 300));
...
function acceptFileTransfer(TargetUser, fileTransferSessionID) {
  var packet = this.newPacket(Service.FXFER_INVITE);
  packet.fields.add(Field.CURRENT_ID, this.sessionData.userData.username)
    .add(Field.TARGET_USER, TargetUser)
...
module.exports.receiveFile = function receiveFile(
  ...
  var packet = this.newPacket(Service.FXFER_RECEIVE);
  packet.fields.add(Field.CURRENT_ID, this.sessionData.userData.username)
    .add(Field.TARGET_USER, TargetUser)
    .add(Field.FT_SESSION_ID, fileTransferSessionID)
...
module.exports.setAwayStatus = function setAwayStatus(awayStatus) {
  if (typeof awayStatus === 'undefined') {
    awayStatus = '0';
  }

  var packet = this.newPacket(Service.SET_AWAY_STATUS);
  packet.fields
    .add(Field.AWAY_STATUS, awayStatus)
    .add(Field.AWAY_MSG, '')
...
module.exports.setCustomAwayStatus = function setCustomAwayStatus(
  awayMsg, customDndStatus, statusLinkType, statusData
) {
  if (typeof customDndStatus === 'undefined') {
    customDndStatus = Consts.CustomDND.Available;
  }
  if (typeof statusLinkType === 'undefined') {
    statusLinkType = Consts.CustomStatusLink.None;
  }
  ...
  var packet = this.newPacket(Service.SET_AWAY_STATUS);
  packet.fields
    .add(Field.AWAY_STATUS, Consts.UserStatus.Custom)
    .add(Field.AWAY_MSG, awayMsg)
    .add(Field.UTF8_FLAG, '1')
    .add(Field.CUSTOM_DND_STATUS, customDndStatus)
    .add(Field.STATUS_LINK_TYPE, statusLinkType)
    .add(Field.STATUS_DATA, statusData);
  this.sendPacket(packet);
  return this;
};
...
module.exports.sendPM = function sendPM(TargetUser, Message, imvId, imvFlag) {
  ...
  var packet = this.newPacket(Service.USER_HAS_MSG);
  packet.fields.add(Field.CURRENT_ID, this.sessionData.userData.username)
    .add(Field.TARGET_USER, TargetUser)
    .add(Field.MSG, Message)
    .add(Field.UTF8_FLAG, '1')
    .add(Field.IMV_ID, imvId)
    .add(Field.IMV_FLAG, imvFlag)
...
function addBuddy(TargetUser, Group, Message, Firstname, Lastname)
{
  ...
  var packet = this.newPacket(Service.ADD_BUDDY);
  /*if (typeof Message  !== 'undefined') */
...
{
  var packet = this.newPacket(Service.BUDDY_AUTHORIZE);
  packet.fields.add(Field.CURRENT_ID, this.sessionData.userData.username)
    .add(Field.TARGET_USER, TargetUser)
    .add(Field.FLAG, '1');
  this.sendPacket(packet);
...
module.exports.logout = function logout() {
  var packet = this.newPacket(Service.USER_LOGOFF);
