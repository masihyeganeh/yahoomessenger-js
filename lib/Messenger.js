...
var Messenger = (function Messenger() {
	...
	return {
		...
		_download: function (ip, path, query, callback) {
			...
			var request = http.request({
				...
				headers: {
					...
					'Cookie': 'T=' + this.userData.cookies.T +'; Y=' + this.userData.cookies.Y + '; B=' + this.userData.cookies.B
				}
			}, function(res) {
		...
		acceptFileTransfer: function(TargetUser, fileTransfer_sessionID) {
			var packet = new Packet(Service.FXFER_INVITE);
			packet.fields.add(Field.CURRENT_ID, this.userData.username);
			packet.fields.add(Field.TARGET_USER, TargetUser);
			packet.fields.add(Field.FT_SESSION_ID, fileTransfer_sessionID);
			packet.fields.add(Field.ACTION_TYPE, '3');
			this.sendPacket(packet);
		},

		declineFileTransfer: function(TargetUser, fileTransfer_sessionID) {
			var packet = new Packet(Service.FXFER_INVITE);
			packet.fields.add(Field.CURRENT_ID, this.userData.username);
			packet.fields.add(Field.TARGET_USER, TargetUser);
			packet.fields.add(Field.FT_SESSION_ID, fileTransfer_sessionID);
			packet.fields.add(Field.ACTION_TYPE, '4');
			this.sendPacket(packet);
		},

		fileTransfer_getFile: function(TargetUser, ip, token, callback) {
			var request = 'token=' + encodeURIComponent(token) +
						'&sender=' + encodeURIComponent(TargetUser) +
						'&recver=' + encodeURIComponent(this.userData.username);

			this._download(ip, '/relay?' + request, '', callback);
		},

		receiveFile: function(TargetUser, fileTransfer_sessionID, fileName, transferType, ip, token) {
			var packet = new Packet(Service.FXFER_RECEIVE);
			packet.fields.add(Field.CURRENT_ID, this.userData.username);
			packet.fields.add(Field.TARGET_USER, TargetUser);
			packet.fields.add(Field.FT_SESSION_ID, fileTransfer_sessionID);
			packet.fields.add(Field.FILE_NAME, fileName);
			packet.fields.add(Field.TRANSFER_TYPE, transferType);
			packet.fields.add(Field.TRANSFER_TAG, ip);
			packet.fields.add(Field.TOKEN, token);
			this.sendPacket(packet);
		},

		// while receiving file
		cancelFileReceive: function(TargetUser, fileTransfer_sessionID) {

			if (typeof fileName  === "undefined") fileName  = 'Untitled.file';

			var packet = new Packet(Service.FXFER_RECEIVE);
			packet.fields.add(Field.CURRENT_ID, this.userData.username);
			packet.fields.add(Field.TARGET_USER, TargetUser);
			packet.fields.add(Field.FT_SESSION_ID, fileTransfer_sessionID);
			packet.fields.add(Field.ERROR_CODE, 'ERROR_CODE');
			this.sendPacket(packet);
		},
...
module.exports = function() {
	...
	MSGR.connection.events.on('receive', function (service, fields, packet) {
		switch (service)
		{
			...
			case Service.FXFER_INVITE:
				var sender = fields.get(Field.SENDER);
				var fileTransfer_sessionID = fields.get(Field.FT_SESSION_ID); //(new Buffer(fields.get(Field.FT_SESSION_ID), 'base64')).toString();
				switch (fields.get(Field.ACTION_TYPE))
				{
					case "1": // File Transfer Invite
						...
						MSGR.fireEvent('fileTransferInvite', {
							'sender': sender,
							'sessionID': fileTransfer_sessionID,
							'thumbnail': thumbnail,
					...
					case "2": // File Transfer Cancelled
						MSGR.fireEvent('fileTransferCancelled', {
							'sender': sender,
							'sessionID': fileTransfer_sessionID
						});
						break;
			...
			case Service.FXFER_SEND:
				...
				else
				{
					...
					var Unknown_502 = fields.get(Field.UNKNOWN_502); // 1

					MSGR.fireEvent('fileTransfer', {
						'sender': sender,
						'fileName': fileName,
			...
			default : // Unhandeled Packets
				MSGR.fireEvent('debug', packet.toString());
